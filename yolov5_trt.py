...
class YoLov5TRT(object):
    ...
    def infer(self, img):
        ...
        input_image, img, origin_h, origin_w = self.preprocess_image(img)
        # batch_image_raw.append(img)
        ...
        results = self.post_process(
            output[: 6001], origin_h, origin_w
        )
        # Draw rectangles and labels on the original image
        if len(results)==0:
            results = [[]]
            #results = np.array(results).unsequeeze()
        else:
            results = results[:,[5,4,0,1,2,3]]
